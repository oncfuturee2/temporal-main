...
type (
	...
	processorImpl struct {
		...
		mapToAckFuture          collection.ConcurrentTxMap[string, *ackFuture] // used to map ES request to ack channel
		logger                  log.Logger
...
func (p *processorImpl) Add(request *client.BulkableRequest, visibilityTaskKey string) *future.FutureImpl[bool] {
	...
	_, isDup, _ := p.mapToAckFuture.PutOrDo(visibilityTaskKey, newFuture, func(key string, existingFuture *ackFuture) error {
...
func (p *processorImpl) bulkBeforeAction(_ int64, requests []elastic.BulkableRequest) {
	...
	for _, request := range requests {
		...
		_, _, _ = p.mapToAckFuture.GetAndDo(visibilityTaskKey, func(key string, ackF *ackFuture) error {
...
func (p *processorImpl) notifyResult(visibilityTaskKey string, ack bool) {
	...
	_ = p.mapToAckFuture.RemoveIf(visibilityTaskKey, func(key string, ackF *ackFuture) bool {
